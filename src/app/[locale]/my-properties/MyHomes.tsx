...
export default function MyHomes({
  ...
  const sortHomes = useCallback(
    (homesToSort: HomeType[]) => {
      let sortedHomes = [...homesToSort]; // Clone the array to avoid in-place sorting
      ...
      switch (sortOption) {
        ...
        case "favorited":
          sortedHomes.sort(
            (a: any, b: any) =>
              (b.favoritedCount || 0) - (a.favoritedCount || 0)
          );
          break;
        case "price":
          sortedHomes.sort(
            (a: any, b: any) => (b.priceUsd || 0) - (a.priceUsd || 0)
          );
          break;
        case "incomplete":
        ...
        <Card>
          ...
          <CardContent className="p-2 xs:p-6">
            ...
            <p className="text-xs text-muted-foreground">
              {cardText.propertiesDescription}
            </p>
          </CardContent>
        </Card>
        <Card>
      ...
      <div className="p-2 px-4 sm:p-4 md:px-6 overflow-y-auto w-full h-full grid grid-cols-1 2xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 justify-center items-center gap-2 md:gap-4 lg:gap-5 xl:gap-5">
        {visibleHomes.map((home: HomeType, index: number) => {
          ...
          return (
            <div
              key={index}
              className={`flex flex-col h-full w-full space-y-2 rounded-lg`}
            >
              <ResizableCard
                home={home}
