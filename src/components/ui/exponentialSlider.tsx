...
const ExponentialSlider = React.forwardRef<
  React.ElementRef<typeof SliderPrimitive.Root>,
  React.ComponentPropsWithoutRef<typeof SliderPrimitive.Root> & {
    minValue: number;
    maxValue: number;
    exponent: number; // Add an exponent prop for controlling the curve
    onValuesChange?: (values: number[]) => void; // Callback for the raw values
  }
>(({ className, value, onValueChange, minValue = 1, maxValue = 30, exponent = 2, onValuesChange, ...props }, ref) => {
  const [tooltipVisible, setTooltipVisible] = React.useState<number | null>(null);
  const { numerals } = React.useContext(LocaleContext);
  ...
  return (
    <SliderPrimitive.Root
      ...
      onValueChange={(newValue) => {
        const exponentialValues = newValue.map(linearToExponential);
        onValuesChange?.(exponentialValues); // Pass raw values to parent
        onValueChange?.(exponentialValues);
      }}
      min={minValue}
      max={maxValue}
      {...props}
...
ExponentialSlider.displayName = SliderPrimitive.Root.displayName;

export { ExponentialSlider };
