...
export default function Listings({ listingKey, typesObject, loginToViewPrice, premiumText }: Props) {
  ...
  return (
    <div className="flex flex-col items-center h-full w-full">
      <div className="grid p-2 px-4 sm:p-4 md:px-6 w-full h-full grid-cols-1 2xs:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 lg:grid-rows-1 xl:grid-cols-5 xl:grid-rows-1 justify-center items-center gap-2 sm:gap-4 lg:gap-5 xl:gap-5">
        {homes.map((home, index) => {
          ...
          return (
            <div
              key={index}
              className={`flex flex-col h-full w-full space-y-2 shadow-lg ${home?.listingType === "premium" ? "shadow-yellow-500/40 dark:shadow-yellow-500/40" : "dark:shadow-white/10"} rounded-lg
