...
def scrape_urbania(location):
    """Scrapes property data from Urbania.pe for a specific location."""
    ...
    for prop_type in property_types:
        ...
        url = f"https://urbania.pe/buscar/venta-de-{prop_type}-en-{location_slug}"
        
        print(f"Scraping Urbania.pe for {prop_type} in {location}: {url}")
        ...
        try:
            ...
            if 'driver' in locals():
                driver.quit()
            continue # Continue to next property type

        # This is a placeholder for the actual scraping logic.
        # We will refine this based on the actual HTML structure of Urbania.pe
        listings = soup.find_all("div", {"data-qa": "posting-card"})

        if not listings:
            print(f"No listings found on Urbania.pe for {prop_type} in {location}.")
            continue
...
if __name__ == "__main__":
    ...
    scraped_data = scrape_urbania(test_location)
    if scraped_data:
